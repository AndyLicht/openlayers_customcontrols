var layertree = [];
layertree = bildlayertree();
$('#layertree').treeview({data: layertree,showCheckbox:true,showOpacity:true,showDeleteIcon:true,showXmlIcon:true,showExtentIcon:true,showLegendIcon:true,});

modification = false;	

//Wenn ein Node angeklickt ->aktiviert wird
$('body').on('nodeChecked','#layertree', function(event,node)
{
    ...
    if(typeof(node['nodes'])=='undefined')
    {
        ...
        {
            //Eltern-Node aktivieren
            parent = $('#layertree').treeview('getParent',node);
            $('#layertree').treeview('checkNode',parent);
        }
        modification = false;
    }
    else
    {
        if(modification == false)
        {
            for (var n = 0; n < node['nodes'].length; ++n)
            {
                layertree.forEach(function(layer) 
                {
                    if (layer.get('name') == node['nodes'][n]['val'])
                    {
                        layer.setVisible(true);
                    };
                });
                $('#layertree').treeview('checkNode',node['nodes'][n]['nodeId']);
            }
        };
    };
});
...
/*
* Funktion zum deaktivieren von Layern und zum deaktivieren von Einträgen
*/
$('body').on('nodeUnchecked','#layertree', function(event,node)
{
	layertree = map.getLayers();
	/*
	* Prüfen ob es sich um ein Kind handelt
	*/
	if(typeof(node['nodes'])=='undefined')
	{
		layertree.forEach(function(layer) 
		...
		//prüfen wieviele noch checked sind, wenn mehr als 1 dann mache nichts, wenn 0 dann uncheck parent
		modification = true;
		if(modification == true)
		{
			parent = $('#layertree').treeview('getParent',node);
			checked = 0;
			for (var n = 0; n < parent['nodes'].length; ++n)
			{
				if(parent['nodes'][n]['state']['checked']==true)
				{
					checked++;
				};			
			}
			if(checked < 1)
			{
				$('#layertree').treeview('uncheckNode',parent);
			}
		}
		modification = false;
	}
	else
	{
		if(modification == false)
		{
			for (var n = 0; n < node['nodes'].length; ++n)
			{
				...
				$('#layertree').treeview('uncheckNode',node['nodes'][n]['nodeId']);
			}
		}
	};
});
...
//Sortable machen
//An dieses Event muss außerdem ein re-render des Layertree erfolgen sowie eine Anpassung der internen Rangfolge 
 //$(function() {
    //$('.list-group').multisortable(); 
 //});
 
 
 //Bibo für sortable vorerst auskommentiert.
 /**
